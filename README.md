...
### 1.2. 각 항목에 대한 웹브라우저에서 임시로 보관하는 작업
...
- 이건 옛날 JS 버전에서만 씁니다
<!-- var : 어디서든 재선언 가능하나 덮어쓰기 위험이 있다. (블록 구분 못함) -->
...
<!-- let (약속된 단어 key word) : 변할 수 있는 값을 저장할때 사용. 재선언은 안 되지만 값은 바꿀 수 있다! 대신 블록 안에서만 유효(일반적인 변수 선언) -->
...
<!-- const : 빈공간이 있으면 알려줌
= 빈공간이 있으면 돌려준다. 절대 바꾸지 않을 값으 저장할 때 사용 (변하지 않는 값) -->
...
<!-- null 은 개발자가 값이 없다고 지정하는 것
undefined 는 값이 없다고 JS가 기본으로 지정하는 것 -->
